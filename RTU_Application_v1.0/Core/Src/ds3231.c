...
float DS3231_GetTemperature(void) {
	...
	// Temperature format:
	// MSB = integer part (two's complement)
	// LSB bits 7-6 = fractional (0.25°C steps)
	temperature = (float) ((int8_t) temp_msb);
	temperature += ((float) ((temp_lsb >> 6) & 0x03)) * 0.25f;
...
uint8_t DS3231_ForceTempConv(void) {
	...
	// Check if conversion is already in progress (BSY bit, bit 2)
	if (!(status & 0x04)) {
...
static uint8_t DS3231_WriteReg(uint8_t reg, uint8_t data) {
	...
	result = I2C1_SendAddr(DS3231_ADDR, 0U);  // Write mode
	if (result != I2C_OK) {
		I2C1_Stop();
		return DS3231_ERROR;
	}
	...
	// Send data
	result = I2C1_WriteByteWaitComplete(data);
	if (result != I2C_OK) {
		I2C1_Stop();
		return DS3231_ERROR;
	}
...
static uint8_t DS3231_WriteMulti(uint8_t reg, uint8_t *data, uint8_t len) {
	...
	// Send all data bytes
	for (i = 0; i < len; i++) {
		result = I2C1_WriteByteWaitComplete(data[i]);
		if (result != I2C_OK) {
			I2C1_Stop();
			return DS3231_ERROR;
		}
