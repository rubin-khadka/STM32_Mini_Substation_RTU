...
static void LCD_SendNibble(uint8_t nibble, uint8_t rs) {
	...
	I2C1_WriteByteWaitComplete(data | LCD_ENABLE);  // E=1
	I2C1_Stop();
	...
	I2C1_Start();
	I2C1_SendAddr(LCD_ADDR, I2C_WRITE);
	I2C1_WriteByteWaitComplete(data & ~LCD_ENABLE);  // E=0
	I2C1_Stop();
}
...
// Display temperature and humidity (integers with one decimal)
void LCD_DisplayReading_Temp(uint8_t temp_int, uint8_t temp_dec,
...
// Display a float with specified decimal places (no leading spaces)
